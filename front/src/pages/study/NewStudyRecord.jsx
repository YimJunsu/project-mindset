// src/pages/study/NewStudyRecord.jsx
...
const NewStudyRecord = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('전송할 데이터:', studyData);
      
      await studyRecordAPI.createStudyRecord(studyData);
      navigate('/study/record');  // 경로 수정
    } catch (err) {
  ...
  return (
    <div className={`max-w-3xl mx-auto my-12 p-6 ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'} rounded-lg shadow-md`}>
      <h1 className="text-2xl font-bold mb-8">새 공부 기록</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <div className="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 dark:bg-gray-700 rounded-md bg-gray-100 dark:bg-gray-600">
            {formData.startTime && formData.endTime ? `${calculateDuration()}분` : '시작 및 종료 시간을 입력해주세요'}
          </div>
        </div>
